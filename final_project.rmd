...
gdppc_ttest_data <- data.frame(fdi_2019$conflict, fdi_2019$gdppc)
...
colnames(gdppc_ttest_data) <- c("conflict", "gdppc")
(ttest <- t.test(gdppc_ttest_data$gdppc ~ gdppc_ttest_data$conflict, conf.level = .99))
```
...
for (i in 1:N) {
  bootstrapped_conflict[i] <- mean(sample(gdppc_ttest_data$gdppc[gdppc_ttest_data$conflict == TRUE],
...
for (i in 1:N) {
  bootstrapped_no_conflict[i] <- mean(sample(gdppc_ttest_data$gdppc[gdppc_ttest_data$conflict == FALSE],
...
for (i in 1:N) {
  ...
  diffvals[i] <- mean(gdppc_ttest_data$gdppc[fakeconflict == TRUE]) -
    mean(gdppc_ttest_data$gdppc[fakeconflict == FALSE])
}

actualdiff <- mean(gdppc_ttest_data$gdppc[gdppc_ttest_data$conflict == TRUE])  -
  mean(gdppc_ttest_data$gdppc[gdppc_ttest_data$conflict == FALSE])
